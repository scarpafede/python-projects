...
Autore: Pasetto Alessandro, Scarpa Federico, Tosi Giacono
Descrizione: Il programma scrive i "bioritmi" di una classe in una tabella html.
"""
...
def calculate_biorhythm(birthdate, target_date):
    ...
    calcolati dalle due date fornite come argomento.
    """
...
def passaggio_html():
    ...
    riporta in una tabella nel file html 'bioritmi_classe'.
    """
    ...
    with open("../html/bioritmi_classe.html", 'w') as filehtml:
        ...
        for line in lines:
            ...
            if float(splitted_line[2]) < 0.0:
                filehtml.write('    <td style="background-color:LightCoral;">' + splitted_line[2] + '</td>\n')
            else:
                filehtml.write('    <td style="background-color:PaleGreen;">' + splitted_line[2] + '</td>\n')

            if float(splitted_line[3]) < 0.0:
                filehtml.write('    <td style="background-color:LightCoral;">' + splitted_line[3] + '</td>\n')
            else:
                filehtml.write('    <td style="background-color:PaleGreen;">' + splitted_line[3] + '</td>\n')

            if float(splitted_line[4]) < 0.0:
                filehtml.write('    <td style="background-color:LightCoral;">' + splitted_line[4] + '</td>\n')
            else:
                filehtml.write('    <td style="background-color:PaleGreen;">' + splitted_line[4] + '</td>\n')
