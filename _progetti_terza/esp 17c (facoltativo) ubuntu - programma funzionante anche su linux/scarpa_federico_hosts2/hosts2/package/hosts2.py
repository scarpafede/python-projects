...
Descrizione: il programma visualizza il contenuto del file hosts privato delle linee con '#' e
                traccia la sua esecuzione nel file execute.log
"""
...
def messaggio():
    ...
    platform = platform_percorso(1)
    if platform == "darwin" or "linux" in platform:
        nome_computer = node()
    elif platform == "win32":
        nome_computer = os.getenv("COMPUTERNAME")
    ...
    f1 = open(percorso_relativo_hosts_deploy(), 'r')
    lines = f1.readlines()
    for line in lines:
        if line[0] != '#':
...
def platform_percorso(platform_o_percorso):
    """
    ...
    if platform == "win32":
        ...
        path = "/etc/hosts"
    if platform_o_percorso == 1:
        return platform
    elif platform_o_percorso == 2:
        return path


def percorso_relativo_hosts_deploy():
    """
    La funzione restituisce il percorso relativo del file hosts (nel deploy)
    """
    path = "../flussi/hosts"
    return path
