...
def converter_csv_to_xml(percorso_csv, percorso_xml, ripetitive_element, root_element):
    """
    Converte un file csv in un file xml, dati i due percorsi, un elemento ripetitivo e una root.
    """
    with (open(percorso_csv, 'r') as file_csv,
          open(percorso_xml, 'w')as file_xml):
        
        file_xml.write('<?xml version="1.0" encoding="UTF-8"?>\n')
        file_xml.write("<" + root_element + ">\n")
...
def main():
    ...
    try:
        ...
        print("Percorso file csv errato o mancante")
        sys.exit()
    # fa terminare il programma se il percorso fornito non è valido

    percorso_csv = percorso_csv.replace('\\', '/')
    root_element = percorso_csv.split("/")[-1].split("_")[0]
    percorso_xml = "../flussi/" + root_element + ".xml"
    ripetitive_element = root_element.removesuffix('s')

    converter_csv_to_xml(percorso_csv, percorso_xml, ripetitive_element, root_element)
